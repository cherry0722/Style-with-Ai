...
if not MONGO_URI or USE_MOCK_DB_ENV:
    ...
    if USE_MOCK_DB_ENV:
        print("[DB] USE_MOCK_DB=true: Loading mock data from JSON.")
    else:
        print("[DB] MONGO_URI not set. Using mock in-memory database for development.")
    print("[DB] To use MongoDB, set MONGO_URI environment variable and USE_MOCK_DB=false.")
    ...
    try:
        ...
        print(f"[DB] Connected to MongoDB: {os.getenv('MONGO_DB', 'style_with_ai')}")
